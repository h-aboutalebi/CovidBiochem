...
csv_handle = CSVHandler(csv_file,useless_cols_list=["to_patient_id"],target_col=target_col)
train, test = train_test_split(csv_handle.df, random_state=42)

clf = lgb.LGBMClassifier(random_state=42)
clf.fit(train.drop(columns=target_col), train[target_col], categorical_feature=csv_handle.cat_cols)
