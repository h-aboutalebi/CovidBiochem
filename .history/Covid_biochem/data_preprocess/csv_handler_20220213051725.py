...
class CSVHandler:
    ...
    def apply_imputation_cat(self):
        ...
        self.df[self.self.cat_cols] = pd.DataFrame(
            imp.fit_transform(self.df[self.self.cat_cols]))

    def apply_imputation_num(self):
        imp = SimpleImputer(missing_values=np.nan, strategy='mean')
        self.df[self.self.num_cols] = pd.DataFrame(
            imp.fit_transform(self.df[self.self.num_cols]))
