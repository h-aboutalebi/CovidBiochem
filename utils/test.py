...
class Test():
    ...
    def cal_correct(self, loader, model, device):
        ...
        total = 0
        prediction = []
        auc_cond = [i * 0.1 for i in range(1, 10)]
        extra_prediction = [[] for i in range(1, 10)]
        target = []
        for data in loader:
            images, labels = data
            # import pdb;pdb.set_trace()
            outputs = model(images.to(device).float())
            self.get_auc( auc_cond, extra_prediction, outputs)
        ...
        precision = tp / (tp + fp)
        recall = tp / (tp + fn)
        self.print_auc(extra_prediction,target,auc_cond)
        return correct, total, precision, recall, mcc

    def get_auc(self, auc_cond, extra_prediction, outputs):
        prob_positive = torch.softmax(outputs, 1).data[:, 1]
        all_zero = torch.zeros(outputs.shape[0]).to(prob_positive.device)
        all_one = torch.ones(outputs.shape[0]).to(prob_positive.device)
        for i,cond in enumerate(auc_cond):
            # import pdb;pdb.set_trace()
            results=torch.where(prob_positive > cond, prob_positive, all_zero)
            final_results=torch.where(results <=0, results, all_one)
            extra_prediction[i].extend(final_results.detach().cpu().numpy().tolist())

    def print_auc(self,extra_prediction,target,auc_cond):
        s = ""
        for i, cond in enumerate(auc_cond):
            # import pdb;pdb.set_trace()
            tn, fp, fn, tp = confusion_matrix(target, extra_prediction[i]).ravel()
            s=s+str(cond)+": "+"tn: "+str(tn)+" fp: "+str(fp)+" fn: "+str(fn)+" tp: "+str(tp)+"| "
        print(s)
